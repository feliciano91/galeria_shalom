document.addEventListener('DOMContentLoaded', function () {
    ...
    function preencherHorarios() {
        ...
        // Busca os horários já agendados no banco de dados
        fetch(`/get_horarios/${dataSelecionada.toISOString().split('T')[0]}`)
            .then(response => response.json())
            .then(horariosOcupados => {
                // Exibe os horários em 10 colunas
                for (let i = 0; i < horariosDisponiveis.length; i++) {
                    if (i % 10 === 0) {
                        var tr = document.createElement('tr');  // Cria uma nova linha para cada 10 horários
                    }
                    ...
                    if (horariosOcupados.includes(horariosDisponiveis[i])) {
                        td.classList.add('indisponivel'); // Marca como indisponível
                    } else {
                        td.classList.add('horario');
                        td.addEventListener('click', () => selecionarHorario(horariosDisponiveis[i])); // Seleção de horário
                    }

                    tr.appendChild(td);

                    if ((i + 1) % 10 === 0 || i === horariosDisponiveis.length - 1) {
                        horarioElement.appendChild(tr); // Adiciona a linha ao elemento
                    }
                }
            })
